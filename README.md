# UC Santa Cruz / CMPM 244 AI in Games

### AI Pirates Team  
Rohit Nagubandi, Ivan Martinez-Arias, Shaoze (Roger) Li

### Motivation
This project is based on and inspired by [Bahar's MiniStS Implementation](https://github.com/iambb5445/MiniSTS) of _Slay the Spire_ and [Chen and Guy's Chaos Cards Paper Implementation](https://github.com/iambb5445/MiniSTS) for card generation within their game _Chaos Cards_. This project is for a class group project within AI in Games, focusing on how to generate unique and playable card and testing their card strengths via _Slay the Spire_ game simulations.  

## Virtual Environment Setup
To find more details on how to set up the environment for MiniStS, more details can be found under the original documentation titled ["How To Run"](./README.md#how-to-run).

## GIGL Function (Card Generation):
In order to run the program, you must enter into the directory and perform this command:
* Enter into the GIGL folder
* Run ```python main.py -n [number of cards] -c [config directory location]``` for the amount of cards generated and the configurations of the card generation
* * _Ex./ ```python3 main.py -n 20 -c configs```_
* Once generation is complete, you should be able to view that the folder ```generated_cards``` is created with the newly generated cards. Ensure that this file path is correct based on the ```card.py``` for generated cards to be incorporated into the game.

## MiniStS Game Playability:
In order to run the game manually, you must ensure the following steps:
* Inside main.py, ensure that ```agent = HumanInput(True)``` is uncommented without other agent variables active.
* * _Note: If you want a Random Bot to play, make sure you do ```agent = RandomBot()``` instead of the above._
* While in main.py, make sure that ```game_state.set_deck(*CardRepo.get_generated_scenario()[1])``` is also uncommented without other agent variables active. This will allow the generated cards to be playable when you run the game.
* Run ```python3 main.py``` to play the game. Inside, the game will provide you the cards in hand that are playable. 
  
# MiniStS Documentation
Below is Bahar Bateni's original documentation for MiniStS. Please check out their own [repository here](https://github.com/iambb5445/MiniSTS) for the original source.
